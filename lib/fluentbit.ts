...
export class MyFluentBitAddOn extends blueprints.addons.HelmAddOn {
  ...
  deploy(clusterInfo: blueprints.ClusterInfo): Promise<Construct> {
    ...
    if (secretProviderClass) {
      // if secret provider class must be created before the helm chart is applied, add dependenncy to enforce the order
  ...
  private setupSecretProviderClass(
    ...
    return new blueprints.addons.SecretProviderClass(
      clusterInfo,
      serviceAccount,
      "my-addon-license-secret-class",
      csiSecret
    );
  }
